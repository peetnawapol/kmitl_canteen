<div class="container-fluids p-5">
    <div class="row">
        <div class="col-6 ">
            <div class="card shadow p-3 mb-5 bg-white rounded">
                <h2>Order</h2>
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col">#</th>
                            <th scope="col">Order By</th>
                            <th scope="col">Menu</th>
                            <th scope="col">Amount</th>
                            <th scope="col">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        include 'connect.php';
                        session_start();
                        $store = $_SESSION['store'];
                        $sql = "SELECT * FROM order_list WHERE customer_name='$store' AND status='NOT_PROCESS' ORDER BY time ASC;";
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0)
                            while ($row = $result->fetch_assoc()) {
                                $order = $row["time"];
                                ?>
                            <tr>
                                <th scope="row"><?php echo $order; ?></th>
                                <td><?php echo $row["order_by"]; ?></td>
                                <td><?php echo $row["food_name"]; ?></td>
                                <td><?php echo $row["qty"]; ?></td>
                                <td><a class="btn btn-primary" role="button"  href="submit.php?id=<?php echo $order;?>">SUBMIT</a>
                                <a class="btn btn-danger" role="button" target="_blank" href="cancel.php?id=<?php echo $order;?>">CANCEL</a></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="col-6 ">
            <div class="card shadow p-3 mb-5 bg-white rounded">
                <h2>Process</h2>
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col">time</th>
                            <th scope="col">Order By</th>
                            <th scope="col">Menu</th>
                            <th scope="col">Amount</th>
                            <th scope="col">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php
                        include 'connect.php';
                        $sql = "SELECT * FROM order_list WHERE customer_name='$store' AND status='PROCESS' ORDER BY time ASC;";
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0)
                            while ($row = $result->fetch_assoc()) {
                                $order = $row["id_order"];
                                ?>
                        <tr>
                            <th scope="row"><?php echo $row['time'];?></th>
                            <td><?php echo $row["order_by"]; ?></td>
                            <td><?php echo $row["food_name"]; ?></td>
                            <td><?php echo $row["qty"]; ?></td>
                            <td><a class="btn btn-success" role="button" href="finish.php?id=<?php echo $order;?>">FINISH</a></td>
                        </tr>
                            <?php } ?>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>